...
(function(){
  ...
  // ---------- TEACHER / TEAM BOARD ----------
  var teamRecs=[];
  async function loadTeam(){ bzDay=todayName(); var mon=monday(); var d=await api({action:'list', table:'Buzz Board', maxRecords:300, sortField:'Date'}); teamRecs=((d&&d.records)||[]).map(function(r){return {f:r.fields};}).filter(function(r){return (sel(r.f['Week Of'])||'').slice(0,10)===mon;}); renderTeam(); }
  function renderTeam(){
  ...
  function boot(){
    ...
    if(isTeach){
      addPanel('buzz', '<div class="step">🐝 The Buzz Board — see who\'s filling their hive this week! 💛</div><div class="bz-days" id="bztDays"></div><div id="bztArea" class="empty">Loading the hive… 🐝</div><div class="step" style="margin-top:20px">🏆 This Week\'s Hive Leaders</div><div class="bz-lead" id="bztLead"></div>');
      addTab('🐝 The Buzz Board', 'linear-gradient(90deg,#ffb13d,#ff6b6b)', 'buzz', loadTeam);
    } else {
      addPanel('buzzt', '<div class="banner">🐝 The Buzz Board — your team\'s week at a glance. Green fills in as they clock in/out and finish their charts. Cheer them on! 💛</div><div class="bz-days" id="bztDays"></div><div id="bztArea" class="empty">Loading the hive… 🐝</div><div class="sect">🏆 This Week\'s Hive Leaders</div><div class="bz-lead" id="bztLead"></div>');
